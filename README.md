...
Static US dumpster-rental directory. Brand name: **Which Dumpster**. Operator: Jordan Craig, craigja88@gmail.com. Planned domain: whichdumpster.com.
...
6. Add the new city to the visible lists by copying a list item in `index.html` and `cities/index.html` (same `data-filter` pattern as existing cities). Miami stays first on the homepage as the featured city unless you intentionally change that.
...
- `index.html` — city search, how it works, size snapshot, quote placeholder (FTC disclosure by the CTA). Featured example city: Miami.
